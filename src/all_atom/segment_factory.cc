...
SegmentFactory::segment_from_pdb(
    ...
    // get 3D coordinates of all residues in segment
    ...
    auto m_elements = std::make_shared<MotifElements>(*rna_structure, *protein_structure, *small_molecules,
                                                      bps, ends);
    _standardize_motif_elements(*m_elements, 0);
...
SegmentFactory::_standardize_motif_elements(
    ...
    if(steric_clashes_1 > 2) {
        ...
        _align_motif_elements_to_frame(base_helix_->get_end(1), m_elements, end_index);
        auto steric_clashes_2 =  _num_steric_clashes(m_elements, *base_helix_);
        if(steric_clashes_2 != 2) {
            LOG_WARNING("SegmentFactory",
                        "When creating a segment from: " + name_ + " with aligned end: " +
                        m_elements.ends[end_index].get_name()->get_str() + " there is no aligment without clashes "
                        " this may lead to issues during building!");
        }
...
SegmentFactory::_num_steric_clashes(
        MotifElements const & m_elements,
        Segment const & seg) {
...
SegmentFactory::_setup_ref_motif() {
    ...
    auto name = std::make_shared<base::SimpleString>(filename.substr(0, filename.length() - 4));
    auto residues = pdb_parser_.parse(path);
    auto rna_structure = get_structure_from_residues(residues->RNA_residues);
    auto x3dna_basepairs = x3dna_.get_basepairs(path);
    auto bps = Basepairs();
    auto ends = get_basepairs_from_x3dna(x3dna_basepairs, *rna_structure)->get_data();
    auto end_ids = base::SimpleStringCOPs();
    _setup_end_ids(*rna_structure, bps, ends, end_ids);

    auto protein_structure = Structure(Residues(), Cutpoints());
    auto small_molecules = Structure(Residues(), Cutpoints());
...
SegmentFactory::_setup_base_helix() {
    ...
    auto aligned_s = _get_aligned_structure(*rna_structure);
    _get_aligned_ends(*aligned_s, ends);
    _remove_beads_from_end_res(ends, *aligned_s);
    ...
    return std::make_shared<Segment>(*rna_structure, bps, ends, end_ids, name,
                                     protein_structure, small_molecules, dot_bracket,
...
SegmentFactory::_remove_beads_from_end_res(
    ...
    for(auto const & end : ends) {
        s.remove_residue_beads(end.get_res1_uuid());
        s.remove_residue_beads(end.get_res2_uuid());
    }
